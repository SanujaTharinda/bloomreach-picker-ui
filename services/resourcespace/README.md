...
|----------|-------------|-------|
| `RS_SCRAMBLE_KEY` | Internal security key | Auto-generated by RS during setup |
| `RS_API_SCRAMBLE_KEY` | API security key | Auto-generated by RS during setup |

> **Note**: These are **auto-generated** by ResourceSpace during initial setup. Only set these if migrating an existing installation.
...
The Dockerfile (based on [official](https://github.com/resourcespace/docker)):
1. Uses Ubuntu 24.04 as base (matches official)
2. Installs all required dependencies (PHP 8.3, Apache, ImageMagick, FFmpeg, OpenCV, etc.)
3. Configures PHP settings (upload size, memory limit, etc.)
4. Checks out ResourceSpace v10.7 from official SVN repository
5. Sets up daily cron job for scheduled tasks
6. **Custom**: Applies our `config/config.php` that reads DB settings from environment variables

### Changes from Official

| Aspect | Official | Our Custom |
|--------|----------|------------|
| `config.php` | Hardcoded values | Reads from env vars |
| Database | Always MariaDB container | Supports Azure MySQL |
| Security keys | Generated during setup | Same (optional env override) |
